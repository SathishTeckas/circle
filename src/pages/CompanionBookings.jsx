...
export default function CompanionBookings() {
  ...
  const { data: bookings = [], isLoading } = useQuery({
    ...
    queryFn: async () => {
      return await base44.entities.Booking.filter({ companion_id: user.id }, '-created_date', 50);
    },
    enabled: !!user?.id
  });
        ...
        {isLoading ? (
          ...
          <div className="text-center py-16">
            ...
            <p className="text-slate-600">
              {activeTab === 'upcoming' 
                ? 'Your confirmed meetups will appear here'
          ...
          <div className="space-y-4">
            {displayBookings.map((booking, idx) => (
              ...
              >
                <BookingCard booking={booking} userRole="companion" />
              </motion.div>
            ))}
          </div>
        )}
